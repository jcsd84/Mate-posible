getwd()
setwd("/home/jcsd/Documents/Data analysis") # LInux
setwd("/Users/home/Dropbox/Matematicamente posible/Data analysis") # Mac

install.packages("effsize")
install.packages("orddom") # Para Probability of superiority 
...
data1 <- read.csv2("BD_eliminatorias_Rusia.csv", sep = ",")
data1

data1$Grupo_analisis[c(17,18)] <- c("2","2")
data1
...
tapply(data1$Goles_Peru,data1$Grupo_analisis, mean)
summary(data$Pases_efectiv)

lapply(split(data1$Peru_pases_tot, data1$Grupo_analisis), max, na.rm = T)
lapply(split(data1$Peru_pases_tot, data1$Grupo_analisis), mean, na.rm = T)

lapply(split(data1$Peru_disp_arco, data1$Grupo_analisis),
       mean, na.rm = T)
lapply(split(data1$Peru_faltas, data1$Grupo_analisis), 
       mean, na.rm = T)
lapply(split(data1$Peru_recupero, data1$Grupo_analisis), 
       mean, na.rm = T)

lapply(split(data1$Peru_pases_tot, data1$SEDE),
       mean, na.rm = T)

lapply(split(data1$Peru_pases_tot, data1$Resultado_g2_e1_p0),
       mean, na.rm = T)

lapply(split(data1$Peru_pases_tot, data1$SEDE),
       mean, na.rm = T)

lapply(split(data1$Goles_Peru, data1$SEDE), 
       mean, na.rm = T)
...
general <- ggplot(data=data1, aes(x=ID, y=Puntos))+
